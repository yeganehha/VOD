...
>
    <div class="flex items-center gap-x-2">
        ...
        @else
            <x-filament::avatar
                src="{{ asset('assets/images/system_avatar.jpeg') }}"
                size="md"
            />
        @endif
        <div class="flex flex-col text-left">
            <span class="font-semibold dark:text-gray-300 text-right">{{ $activity->causer?->name ? $activity->causer?->name. ' ( '. $activity->causer->type->getLabel() .' '.$activity->causer->username .')'  :  "اتومات - سیستمی" }}</span>
            <div class="flex text-left gap-4">
                <span class="text-xs text-gray-700 dark:text-gray-200">
                    {{ $activity->created_at->toJalali()->format(__('filament-activity-log::activities.time_format')) }}
                </span>
                @if($activity->properties->has('agent'))
                    <span class="text-xs text-gray-700 dark:text-gray-200">
                    IP: {{ optional($activity->properties->get('agent'))['ip'] }}
                    </span>
{{--                    <span class="text-xs text-gray-700 dark:text-gray-200">--}}
{{--                    IP: {{ optional($activity->properties->get('agent'))['ip-v2'] }}--}}
{{--                    </span>--}}
                    <span class="text-xs text-gray-700 dark:text-gray-200">
                    مرورگر: {{ optional($activity->properties->get('agent'))['browser'] }}
                    </span>
                    <span class="text-xs text-gray-700 dark:text-gray-200">
                    سیستم عامل: {{ optional($activity->properties->get('agent'))['os'] }}
                    </span>
                @endif
            </div>
        </div>
